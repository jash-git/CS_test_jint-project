...
function Main() {
    ...
    */
    ...
    strbuf = '商品名稱                       數  量    價  格';//48=32+6+4+6
    ESC_Value.push(ecTEXT_ALIGN_LEFT + strbuf + ecFREE_LINE);//文字靠左 + 分隔線 + 換行
    ...
    if (json_obj.order_items != null) {
        for (var i = 0; i < json_obj.order_items.length; i++) {
            ...
            if ((json_obj.order_items[i].product_type == 'P') || (json_obj.order_items[i].product_type == 'K')) {//一般餐點和包材
                ...
                space = "";
                var amount = "" + json_obj.order_items[i].amount;
                spaceCount = 6 - Wlen(count);
                for (var j = 0; j < spaceCount; j++) {
                    space += " ";
                }
                ...
                //48字
                space = "";
                spaceCount = 48 - Wlen(product_name) - Wlen(count) - 4 - Wlen(amount);
                for (var j = 0; j < spaceCount; j++) {
                    space += " ";
                }
                strbuf = product_name + space + count + "    " + amount;//48=32+6+4+6
