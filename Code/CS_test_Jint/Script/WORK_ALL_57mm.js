...
function Normal() {//正常模式
    ...
    //單號;文字靠左 + 放大 + 單號 + 換行
    strbuf = ShiftSpace + '單號(' + json_obj.order_type_name + ') :' + json_obj.call_num;//json_obj.call_num
    ...
    if (PrinterParms.print_ticket_memo != "N") {
        strbuf = ShiftSpace + "訂單備註: " + json_obj.remarks;

        //訂單備註加大
        if (PrinterParms.big_memo_font != "N") {
            strbuf = ShiftSpace + "訂單備註: " + ecBIG_ON + json_obj.remarks + ecBIG_OFF;
        }
